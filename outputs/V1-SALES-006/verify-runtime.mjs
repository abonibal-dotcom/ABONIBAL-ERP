...
const port = 62006;
const debugPort = 63006;
const appUrl = `http://127.0.0.1:${port}/`;
const profileDir = resolve(tmpdir(), `abonibal-v1-sales-006-${mode}-chrome-profile`);
...
try {
    ...
    if (mode === "baseline") {
        await runSales006BaselineVerification(unauthenticatedRouteSummary);
    } else {
...
async function runBaselineVerification(unauthenticatedRouteSummary) {
    ...
    assert(runtime.result === "PASS", "V1-SALES-006 baseline runtime verification failed.");

}

async function runSales006BaselineVerification(unauthenticatedRouteSummary) {
    ...
    const baselineSummary = await collectIssueAfterSummary(
        setup,
        failedDraft,
        failedIssue,
        successfulDraft,
    ...
    baselineSummary.issuedInvoiceVisibleAfterReload =
        reloadDom.issuedInvoiceRowVisible;
    baselineSummary.stockMovementIdVisibleInUi =
        reloadDom.stockMovementIdVisible;
    baselineSummary.invoiceLineAuditRows =
        reloadDom.invoiceLineAuditRows;
    baselineSummary.cancellationUiAbsent =
        reloadDom.cancelActionAvailable === false;
    ...
    runtime.gates = {
        ...
        draftUiWorks: invoiceDomBefore.invoiceDraftPageVisible,
        ...
        issueFlowWorks:
            successfulIssue.issueSucceeded
            && baselineSummary.reloadIssuedInvoiceStatus === "issued",
        issuedInvoiceVisibleAfterReload: reloadDom.issuedInvoiceRowVisible,
        issuedInvoiceStatusDisplayed: reloadDom.issuedInvoiceStatusDisplayed,
        stockMovementIdNotVisibleBeforeMission:
            reloadDom.stockMovementIdVisible === false,
        invoiceLineAuditMissingBeforeMission:
            reloadDom.invoiceLineAuditRows === 0,
        cancelActionMissingBeforeMission: reloadDom.cancelActionAvailable === false,
...
async function runAfterVerification(unauthenticatedRouteSummary) {
    ...
    const failedDraft = await submitDraftWithQuantity(
        setup.activeProductId,
        9,
        "V1-SALES-006 failed issue"
    );
    const failedIssue = await issueDraftViaUi(failedDraft.createdInvoiceId);
    const successfulDraft = await submitDraftWithQuantity(
        setup.activeProductId,
        2,
        "V1-SALES-006 successful issue"
    );
    const successfulIssue = await issueDraftViaUi(
        successfulDraft.createdInvoiceId
    );
    ...
    afterSummary.issuedInvoiceVisibleAfterReload =
        reloadDom.issuedInvoiceRowVisible;
    afterSummary.invoiceStatusDisplayed =
        reloadDom.issuedInvoiceStatusDisplayed;
    afterSummary.invoiceNumberDisplayed =
        reloadDom.issuedInvoiceNumberDisplayed;
    afterSummary.invoiceTotalDisplayed =
        reloadDom.issuedInvoiceTotalDisplayed;
    afterSummary.invoiceIssuedAtDisplayed =
        reloadDom.issuedInvoiceIssuedAtDisplayed;
    afterSummary.invoiceLineProductSnapshotDisplayed =
        reloadDom.invoiceLineProductSnapshotDisplayed;
    afterSummary.invoiceLineQuantityDisplayed =
        reloadDom.invoiceLineQuantityDisplayed;
    afterSummary.invoiceLineUnitPriceDisplayed =
        reloadDom.invoiceLineUnitPriceDisplayed;
    afterSummary.invoiceLineTotalDisplayed =
        reloadDom.invoiceLineTotalDisplayed;
    afterSummary.stockMovementIdVisibleInUi =
        reloadDom.stockMovementIdVisible;
    afterSummary.stockMovementIdData =
        reloadDom.stockMovementIdData;
    const dom = await collectAfterDomEvidence(invoiceDomBefore, reloadDom);
    const storage = await collectStorageEvidence();
    ...
    runtime.gates = {
        ...
        invoiceListReadViewRenders: reloadDom.invoiceDraftPageVisible,
        issuedInvoiceAppears: reloadDom.issuedInvoiceRowVisible,
        issuedStatusDisplayed: reloadDom.issuedInvoiceStatusDisplayed,
        invoiceNumberDisplayed: reloadDom.issuedInvoiceNumberDisplayed,
        invoiceTotalDisplayed: reloadDom.issuedInvoiceTotalDisplayed,
        invoiceIssuedAtDisplayed: reloadDom.issuedInvoiceIssuedAtDisplayed,
        invoiceLineProductSnapshotDisplayed:
            reloadDom.invoiceLineProductSnapshotDisplayed,
        invoiceLineQuantityUnitPriceTotalDisplayed:
            reloadDom.invoiceLineQuantityDisplayed
            && reloadDom.invoiceLineUnitPriceDisplayed
            && reloadDom.invoiceLineTotalDisplayed,
        stockMovementIdVisibleOrTraceable:
            reloadDom.stockMovementIdVisible
            && reloadDom.stockMovementIdData
        ...
        saleDeductionAccountMatches:
            afterSummary.saleDeductionAccountMatches,
        ...
        issuedInvoiceNotEditableAfterReload:
            reloadDom.issuedInvoiceEditActionAvailable === false,
    ...
    writeFileSync(
        resolve(outputDir, "issued-invoice-audit-summary.json"),
        JSON.stringify(afterSummary, null, 2)
    );
    assert(runtime.result === "PASS", "V1-SALES-006 after runtime verification failed.");
...
async function prepareAfterVerificationFixtures() {
    ...
    return evaluate(cdp, `(async () => {
        ...
        const activeProductId = "v1-sales-006-active-product";
        const deletedProductId = "v1-sales-006-deleted-product";
        ...
        const activeProduct = {
            id: activeProductId,
            sku: "V1-SALES-006-ACTIVE",
            barcode: "V1-SALES-006-ACTIVE",
            name: "V1 Sales 006 Active Product",
            englishName: "V1 Sales 006 Active Product",
        ...
        const deletedProduct = {
            ...activeProduct,
            id: deletedProductId,
            sku: "V1-SALES-006-DELETED",
            barcode: "V1-SALES-006-DELETED",
            name: "V1 Sales 006 Deleted Product",
            isDeleted: true,
            deletedBy: userId,
            deletedAt: now
        };
        ...
        const openingBalanceResult = inventoryService.addMovement({
            ...
            quantityDelta: 5,
            reason: "V1-SALES-006 opening balance",
...
async function collectInvoiceDraftDomEvidence() {
    ...
    return evaluate(cdp, `(() => {
        ...
        return {
            ...
            activeProductSelectable: optionValues.includes("v1-sales-006-active-product"),
            softDeletedProductSelectable: optionValues.includes("v1-sales-006-deleted-product"),
...
async function submitValidDraft(productId) {
    ...
    return evaluate(cdp, `(async () => {
        ...
        discount.value = "1.5";
        tax.value = "0.5";
        notes.value = "V1-SALES-006 create";
        customer.dispatchEvent(new Event("input", { bubbles: true }));
        quantity.dispatchEvent(new Event("input", { bubbles: true }));
        unitPrice.dispatchEvent(new Event("input", { bubbles: true }));
        ...
        const created = invoices.find(invoice => invoice.notes === "V1-SALES-006 create");
        if (!created) {
        ...
        return {
            ...
            createdLineProductSnapshotMatches:
                line?.productNameSnapshot === "V1 Sales 006 Active Product",
...
async function collectIssueDomEvidence(failedInvoiceId, issuedInvoiceId) {
    ...
    return evaluate(cdp, `(() => {
        ...
        const issuedRow = document.querySelector(
            ".invoice-draft-row[data-invoice-id='${issuedInvoiceId}']"
        );
        const issuedLineRows = Array.from(
            document.querySelectorAll(
                ".invoice-line-audit-row[data-invoice-id='${issuedInvoiceId}']"
            )
        );
        const issuedLine = issuedLineRows[0] ?? null;
        const stockMovementText =
            issuedLine
                ?.querySelector(".invoice-line-stock-movement-id")
                ?.textContent
                ?.trim()
            ?? "";
        ...
        return {
            ...
            activeProductSelectable: Boolean(
                document.querySelector("#invoice-product-select option[value='v1-sales-006-active-product']")
            ),
            softDeletedProductSelectable: Boolean(
                document.querySelector("#invoice-product-select option[value='v1-sales-006-deleted-product']")
            ),
            ...
            issuedInvoiceRowVisible: Boolean(issuedRow),
            issuedInvoiceStatusDisplayed:
                issuedRow?.textContent?.toLowerCase().includes("issued") ?? false,
            issuedInvoiceNumberDisplayed:
                Boolean(issuedRow?.textContent?.includes("INV-")),
            issuedInvoiceTotalDisplayed:
                Boolean(issuedRow?.getAttribute("data-invoice-total")),
            issuedInvoiceIssuedAtDisplayed:
                Boolean(issuedRow?.getAttribute("data-issued-at")),
            invoiceLineAuditRows: issuedLineRows.length,
            invoiceLineProductSnapshotDisplayed:
                Boolean(
                    issuedLine
                        ?.querySelector(".invoice-line-product-snapshot")
                        ?.textContent
                        ?.trim()
                ),
            invoiceLineQuantityDisplayed:
                Boolean(
                    issuedLine
                        ?.querySelector(".invoice-line-quantity")
                        ?.textContent
                        ?.trim()
                ),
            invoiceLineUnitPriceDisplayed:
                Boolean(
                    issuedLine
                        ?.querySelector(".invoice-line-unit-price")
                        ?.textContent
                        ?.trim()
                ),
            invoiceLineTotalDisplayed:
                Boolean(
                    issuedLine
                        ?.querySelector(".invoice-line-total")
                        ?.textContent
                        ?.trim()
                ),
            stockMovementIdVisible:
                Boolean(stockMovementText)
                && stockMovementText !== "Not issued"
                && stockMovementText !== "Missing movement",
            stockMovementIdText: stockMovementText,
            stockMovementIdData:
                issuedLine?.getAttribute("data-stock-movement-id") ?? "",
            issuedInvoiceEditActionAvailable: buttons.some(button =>
                button.action === "edit-invoice-draft"
...
async function collectIssueAfterSummary(
    ...
    return evaluate(cdp, `(async () => {
        ...
        return {
            ...
            saleDeductionAccountMatches:
                saleDeduction?.accountId === issuedInvoice?.accountId,
            saleDeductionReferenceType: saleDeduction?.referenceType ?? null,
            saleDeductionReferenceId: saleDeduction?.referenceId ?? null,
            issuedInvoiceNumber: issuedInvoice?.invoiceNumber ?? null,
            issuedInvoiceTotal: issuedInvoice?.total ?? null,
            issuedInvoiceIssuedAt: issuedInvoice?.issuedAt ?? null,
            invoiceLineProductSnapshot:
                issuedInvoice?.lines[0]?.productNameSnapshot ?? null,
            invoiceLineQuantity:
                issuedInvoice?.lines[0]?.quantity ?? null,
            invoiceLineUnitPrice:
                issuedInvoice?.lines[0]?.unitPrice ?? null,
            invoiceLineTotal:
                issuedInvoice?.lines[0]?.lineTotal ?? null,
            lineStockMovementId:
            ...
            productScopedKeySanitized: "products:" + await sanitizeValue(accountId),
            productScopedHashBefore: ${JSON.stringify(setup.productScopedHashBefore)},
            productScopedHashAfter: await nullableSha256(productRaw),
            productQuantityAfter: activeProduct?.quantity ?? null,
            legacyProductHashBefore: ${JSON.stringify(setup.legacyProductHashBefore)},
            legacyProductHashAfter: await nullableSha256(legacyProductRaw),
            stockMovementCountBeforeVerification: ${JSON.stringify(setup.stockMovementCountBefore)},
            stockMovementCountAfterVerification: stockMovements.length,
...
async function updateCreatedDraft(invoiceId) {
    ...
    return evaluate(cdp, `(async () => {
        ...
        const form = document.getElementById("invoice-draft-form");
        quantity.value = "3";
        notes.value = "V1-SALES-006 update";
        quantity.dispatchEvent(new Event("input", { bubbles: true }));
        notes.dispatchEvent(new Event("input", { bubbles: true }));
        form.dispatchEvent(new SubmitEvent("submit", {
            bubbles: true,
...
async function reloadApplication(connection) {

    await connection.send("Page.navigate", { url: appUrl });
    await waitForSelector(connection, "body", 30000);
