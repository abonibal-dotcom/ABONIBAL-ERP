...
const port = 62006;
const debugPort = mode === "baseline" ? 63018 : 63019;
...
const acceptedProfileDir = resolve(
    tmpdir(),
    "abonibal-v1-sales-013-after-chrome-profile"
);
const profileDir = resolve(
    tmpdir(),
    `abonibal-v1-sales-014-${mode}-chrome-profile`
);
...
const runtime = {
    ...
    profileSource: "accepted V1-SALES-013 after-runtime profile copy",
    env: {
...
try {
    ...
    assert(
        ...
        "Accepted V1-SALES-013 after-runtime Chrome profile is missing."
    );
...
async function runBaselineVerification() {
    ...
    assert(runtime.result === "PASS", "V1-SALES-014 baseline runtime verification failed.");
}
...
async function runAfterVerification() {
    ...
    assert(runtime.result === "PASS", "V1-SALES-014 after runtime verification failed.");
}
...
async function runLifecycleScenario() {
    return evaluate(cdp, `(async () => {
        ...
        const createResult = invoiceService.createDraft({
            ...
            lines: [lineInput(1)],
            notes: "V1-SALES-014 lifecycle draft",
        });
        ...
        const updateResult = invoiceService.updateDraft(draft.id, {
            lines: [lineInput(2)],
            notes: "V1-SALES-014 lifecycle draft updated",
        });
        ...
        const failedDraftResult = invoiceService.createDraft({
            ...
            notes: "V1-SALES-014 insufficient quantity fixture",
        });
        ...
        const draftCancelResult = invoiceService.markCancelled(
            failedDraftResult.invoice.id,
            "V1-SALES-014 draft cancellation should remain blocked"
        );
        ...
        const cancelResult = invoiceService.markCancelled(
            issuedInvoice.id,
            "V1-SALES-014 lifecycle cancellation"
        );
        ...
        const duplicateCancelResult = invoiceService.markCancelled(
            cancelledInvoice.id,
            "V1-SALES-014 duplicate cancellation should remain blocked"
        );
        ...
        const returnDraftResult = invoiceService.createDraft({
            ...
            lines: [lineInput(2)],
            notes: "V1-SALES-014 return UI invoice fixture",
        });
    ...
    return evaluate(cdp, `(async () => {
        ...
        const draftResult = draftInvoice
            ? invoiceService.markCancelled(
                draftInvoice.id,
                "V1-SALES-014 draft cancellation should fail"
            )
        ...
        const missingResult = invoiceService.markCancelled(
            ...
            "V1-SALES-014 missing invoice should fail"
        );
...
async function exerciseDuplicateCancellationCase() {
    return evaluate(cdp, `(async () => {
        ...
        const duplicateResult = invoiceService.markCancelled(
            cancelledInvoice.id,
            "V1-SALES-014 duplicate cancellation should fail"
        );
        const movementCountAfter = inventoryService.getAll().length;
...
function writeInvoiceLifecycleRegressionSummary(summary) {
    writeFileSync(
        resolve(outputDir, "sales-lifecycle-returns-regression-summary.json"),
        JSON.stringify(summary, null, 2)
    );
}
