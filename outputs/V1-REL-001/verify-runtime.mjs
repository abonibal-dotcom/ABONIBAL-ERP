...
const port = 62007;
const debugPort = mode === "baseline" ? 63020 : 63021;
...
const acceptedProfileDir = resolve(
    tmpdir(),
    "abonibal-v1-sales-014-after-chrome-profile"
);
const profileDir = resolve(
    tmpdir(),
    `abonibal-v1-rel-001-${mode}-chrome-profile`
);
...
const authTestEmail =
    ...
    ?? process.env.ABONIBAL_AUTH_TEST_EMAIL
    ?? "";
const authTestPassword =
    localEnv.ABONIBAL_AUTH_TEST_PASSWORD
    ?? process.env.ABONIBAL_AUTH_TEST_PASSWORD
    ?? "";
...
const runtime = {
    ...
    scenario: mode === "baseline"
        ? "baseline sales lifecycle including returns regression evidence"
        : "after sales lifecycle including returns regression verification",
    url: appUrl,
    verificationTool: "Chrome DevTools Protocol direct WebSocket client",
    profileSource: "accepted V1-SALES-014 after-runtime profile copy",
    env: {
        ...
        authTestEmailPresent: Boolean(authTestEmail),
        authTestPasswordPresent: Boolean(authTestPassword),
        envTracked,
...
try {
    ...
    assert(
        ...
        "Accepted V1-SALES-014 after-runtime Chrome profile is missing."
    );
    ...
    await waitForSelector(cdp, "#app", 60000);

    await ensureAuthenticatedSession(cdp);
...
async function runBaselineVerification() {
    ...
    await navigateRoute(cdp, "login");
    await waitForSelector(cdp, "#login-page", 30000);
    const loginDom = await collectLoginDomEvidence();

    await navigateRoute(cdp, "dashboard");
    await waitForSelector(cdp, "h1", 30000);
    const dashboardDom = await collectDashboardDomEvidence();
    ...
    const dom = await collectDomEvidence({
        loginPageVisible: loginDom.loginPageVisible,
        dashboardVisible: dashboardDom.dashboardVisible,
        mainNavigationVisible: dashboardDom.mainNavigationVisible,
    ...
    runtime.gates = {
        loginPageLoads: loginDom.loginPageVisible,
        dashboardLoads: dashboardDom.dashboardVisible,
        mainNavigationRenders: dashboardDom.mainNavigationVisible,
        ...
        issuedInvoicePresenceRecorded:
            typeof before.issuedInvoiceExists === "boolean",
        saleDeductionTraceRecorded:
            typeof before.referencedSaleDeductionExists === "boolean",
        ...
        cancellationCapabilityRecorded:
            typeof before.markCancelledReversalSafe === "boolean",
        referencedSaleDeductionStatusRecorded:
            typeof before.referencedMovementTypeOk === "boolean"
            && typeof before.referencedMovementNegative === "boolean"
            && typeof before.referencedMovementAccountOk === "boolean"
            && typeof before.referencedMovementProductOk === "boolean",
    ...
    assert(runtime.result === "PASS", "V1-REL-001 baseline runtime verification failed.");
}

async function runAfterVerification() {
    const before = await collectBaselineSnapshot();
    const authFlow = await exerciseAuthLoginLogoutFlow();
    ...
    await waitForSelector(cdp, ".products-page", 30000);
    const productsRouteDom = await collectRouteDomEvidence();
    const productInventory = await runProductInventoryRegressionScenario();

    await navigateRoute(cdp, "products");
    await waitForSelector(cdp, ".products-page", 30000);
    const productSearchDom = await exerciseProductSearchFilterScenario(
        productInventory.productId,
        productInventory.updatedProductName
    );
    Object.assign(productInventory, productSearchDom);

    await navigateRoute(cdp, "inventory");
    await waitForSelector(cdp, "#inventory-page", 30000);
    const inventoryRegressionDom = await collectInventoryRegressionDomEvidence(
        productInventory.productId
    );
    Object.assign(productInventory, inventoryRegressionDom);

    const productDelete = await runProductSafeDeleteScenario(
        productInventory.productId
    );
    Object.assign(productInventory, productDelete);

    await navigateRoute(cdp, "products");
    await waitForSelector(cdp, ".products-page", 30000);
    const productAfterDeleteDom = await collectProductAfterDeleteDomEvidence(
        productInventory.productId
    );
    Object.assign(productInventory, productAfterDeleteDom);
    ...
    Object.assign(lifecycle, {
        ...
        availabilityAfterReloadMatchesLedger:
            ...
            === lifecycle.availabilityBeforeReturn,
    });

    const returnFlow = await exerciseReturnUiScenario(lifecycle);
    Object.assign(lifecycle, returnFlow);
    ...
    const returnReloadEvidence = await collectReturnReloadEvidence(lifecycle);
    Object.assign(lifecycle, {
        reloadPreservesInvoiceReturnRecord:
            returnReloadEvidence.reloadPreservesInvoiceReturnRecord,
        reloadPreservesReturnSaleReturn:
            returnReloadEvidence.reloadPreservesReturnSaleReturn,
        reloadPreservesReturnAudit:
            returnReloadEvidence.reloadPreservesReturnAudit,
        availabilityAfterReturnReload:
            returnReloadEvidence.availabilityAfterReturnReload,
        availabilityAfterReloadMatchesReturnLedger:
            returnReloadEvidence.availabilityAfterReturnReload
            === lifecycle.availabilityAfterReturn,
    });
    ...
    const storage = sanitizeLifecycleStorageSnapshot(
        before,
        lifecycle,
        productInventory
    );
    ...
    summary.screenshotList = await captureVisualSnapshots();
    ...
    runtime.gates = {
        loginPageRenders: authFlow.loginPageRenders,
        validLoginSucceeds: authFlow.validLoginSucceeds,
        logoutClearsAuthenticatedAccess:
            authFlow.logoutClearsAuthenticatedAccess,
        protectedRoutesBlockUnauthenticated:
            authFlow.protectedRoutesBlockUnauthenticated,
        loginRouteRemainsPublic: authFlow.loginRouteRemainsPublic,
        ...
        accountIdNotProviderUserId:
            ...
            && before.providerUserIdPresent
            && !before.accountIdEqualsProviderUserId,
        dashboardRouteRenders: dashboardDom.dashboardVisible,
        mainNavigationRenders: dashboardDom.mainNavigationVisible,
        productsNavigationWorks: productsRouteDom.productsPageVisible,
        inventoryNavigationWorks: inventoryRouteDom.inventoryPageVisible,
        invoicesNavigationWorks: beforeDom.invoicePageVisible,
        noBrokenRouteNavigation:
            dashboardDom.dashboardVisible
            && productsRouteDom.productsPageVisible
            && inventoryRouteDom.inventoryPageVisible
            && beforeDom.invoicePageVisible,
        noBlankPage:
            dashboardDom.bodyTextLength > 0
            && productsRouteDom.productsPageVisible
            && inventoryRouteDom.inventoryPageVisible
            && beforeDom.invoicePageVisible,
        productListReadsScopedStorage:
            productInventory.productScopedKeyMatches,
        productCreateWorks: productInventory.productCreateWorks,
        productEditWorks: productInventory.productEditWorks,
        productSearchFilterWorks: productInventory.productSearchFilterWorks,
        productSafeDeleteWorks: productInventory.productSafeDeleteWorks,
        softDeletedProductHiddenFromUi:
            productInventory.softDeletedProductHiddenFromUi,
        productReloadPersistenceWorks:
            productInventory.productReloadPersistenceWorks,
        inventoryRouteProtected: before.invoiceRouteAccess === "protected",
        inventoryPageRenders: productInventory.inventoryPageVisible,
        manualOpeningBalanceWorks:
            productInventory.manualOpeningBalanceWorks,
        manualAdjustmentWorks: productInventory.manualAdjustmentWorks,
        currentStockViewRenders: productInventory.currentStockViewRenders,
        movementHistoryRenders: productInventory.movementHistoryRenders,
        currentQuantityEqualsLedger:
            productInventory.currentQuantityEqualsLedger,
        voidedMovementsExcluded:
            productInventory.voidedMovementsExcluded,
        stockAvailabilityGateCorrect:
            productInventory.stockAvailabilityGateCorrect,
        stockAvailabilityBlocksOverRequest:
            productInventory.stockAvailabilityBlocksOverRequest,
        salesLifecycleFixtureReady:
            productInventory.salesLifecycleFixtureReady,
        inventoryReloadPersistenceWorks:
            productInventory.inventoryReloadPersistenceWorks,
        productRecordsNotMutatedByInventory:
            productInventory.productQuantityNotUpdatedByInventory,
        productQuantityNotUpdatedByInventory:
            productInventory.productQuantityNotUpdatedByInventory,
        ...
        returnUiRendersForIssuedInvoice:
            lifecycle.returnUiRendersForIssuedInvoice,
        returnUiDoesNotRenderForDraftInvoice:
            lifecycle.returnUiDoesNotRenderForDraftInvoice,
        returnUiDoesNotRenderForCancelledInvoice:
            lifecycle.returnUiDoesNotRenderForCancelledInvoice,
        returnableInvoiceLinesRender:
            lifecycle.returnableInvoiceLinesRender,
        remainingReturnableDisplayed:
            lifecycle.remainingReturnableDisplayed,
        invalidReturnQuantityRejected:
            lifecycle.invalidReturnQuantityRejected,
        overReturnRejected: lifecycle.overReturnRejected,
        duplicateExcessiveReturnRejected:
            lifecycle.duplicateExcessiveReturnRejected,
        validPartialReturnCreatesRecord:
            lifecycle.validPartialReturnCreatesRecord,
        validPartialReturnExecutes:
            lifecycle.validPartialReturnExecutes,
        returnSaleReturnMovementCreated:
            lifecycle.returnSaleReturnMovementCreated,
        returnSaleReturnReferenceTypeInvoiceReturn:
            lifecycle.returnSaleReturnReferenceTypeInvoiceReturn,
        returnStockMovementIdSet:
            lifecycle.returnStockMovementIdSet,
        returnStockMovementIdReferencesMovement:
            lifecycle.returnStockMovementIdReferencesMovement,
        returnSaleReturnReferencesOriginal:
            lifecycle.returnSaleReturnReferencesOriginal,
        availableStockIncreasesByReturnedQuantity:
            lifecycle.availableStockIncreasesByReturnedQuantity,
        remainingReturnableDecreasesCorrectly:
            lifecycle.remainingReturnableDecreasesCorrectly,
        previousReturnsDisplayAfterReload:
            lifecycle.previousReturnsDisplayAfterExecution
            && lifecycle.reloadPreservesReturnAudit,
        reloadPreservesInvoiceReturnRecord:
            lifecycle.reloadPreservesInvoiceReturnRecord,
        reloadPreservesReturnSaleReturn:
            lifecycle.reloadPreservesReturnSaleReturn,
        availabilityAfterReloadMatchesReturnLedger:
            lifecycle.availabilityAfterReloadMatchesReturnLedger,
        noProductCrudBehaviorChanged:
            lifecycle.productHashBefore === lifecycle.productHashAfter
            && lifecycle.productHashUnchangedByReturn,
        ...
        legacyProductsUnchanged:
            ...
            && lifecycle.legacyProductHashUnchangedByReturn,
        stockMovementsOnlyExpectedChanges:
            lifecycle.stockMovementsOnlyExpectedChanges
            && lifecycle.movementCountAfterReturn
                === lifecycle.movementCountBefore + 4
            && lifecycle.movementCountAfterDuplicateReturn
                === lifecycle.movementCountAfterReturn,
        invoicesOnlyExpectedLifecycleWrites:
            lifecycle.invoicesOnlyExpectedLifecycleWrites
            && lifecycle.invoiceHashUnchangedByReturn,
        invoiceReturnsOnlyExpectedRecords:
            lifecycle.invoiceReturnsUnchangedBeforeReturnUi
            && lifecycle.invoiceReturnCountAfterReturn
                === lifecycle.invoiceReturnCountBefore + 1
            && lifecycle.invoiceReturnCountAfterDuplicate
                === lifecycle.invoiceReturnCountAfterReturn,
        noInvoiceHardDelete:
            lifecycle.invoiceCountAfter >= lifecycle.invoiceCountBefore,
        consoleErrorsZero: runtime.consoleErrors === 0,
        pageExceptionsZero: runtime.pageExceptions === 0,
        envUntracked: !runtime.env.envTracked,
        screenshotsCaptured:
            Array.isArray(summary.screenshotList)
            && summary.screenshotList.length === 8,
    };
    runtime.result = allGatesPass(runtime.gates) ? "PASS" : "FAIL";

    await writeArtifacts(afterDom, storage);
    writeFullV1ReleaseCandidateSummary(summary);
    assert(runtime.result === "PASS", "V1-REL-001 after runtime verification failed.");
}

async function exerciseAuthLoginLogoutFlow() {
    await navigateRoute(cdp, "login");
    await waitForSelector(cdp, "#login-page", 30000);
    const loginDom = await collectLoginDomEvidence();

    assert(authTestEmail, "ABONIBAL_AUTH_TEST_EMAIL is required for release login verification.");
    assert(authTestPassword, "ABONIBAL_AUTH_TEST_PASSWORD is required for release login verification.");

    const logoutEvidence = await evaluate(cdp, `(async () => {
        ...
        await Container.get("router").navigate("dashboard");
        return {
            stateStatus: authStateService.getState().status,
            loginPageVisible: Boolean(document.querySelector("#login-page")),
            dashboardVisible: Boolean(document.querySelector("#dashboard-page"))
        };
    })()`, true);

    const loginEvidence = await evaluate(cdp, `(async () => {
        const { Container } = await import("/src/core/Container.ts");
        const { getAuthStateService } =
            await import("/src/modules/auth/AuthRuntime.ts");
        const authStateService = getAuthStateService();
        await authStateService.signIn({
            email: ${JSON.stringify(authTestEmail)},
            password: ${JSON.stringify(authTestPassword)}
        });
        await Container.get("router").navigate("dashboard");
        ...
        return {
            ...
            accountIdPresent:
                ...
                && Boolean(state.session.account.id),
            userAccountIdPresent:
                state.status === "authenticated"
                && Boolean(state.session.user.accountId),
            accountIdEqualsUserAccountId:
                state.status === "authenticated"
                && state.session.account.id === state.session.user.accountId,
            dashboardVisible: Boolean(document.querySelector("h1")),
        };
    })()`, true);
    ...
    return {
        loginPageRenders: loginDom.loginPageVisible,
        logoutClearsAuthenticatedAccess:
            logoutEvidence.stateStatus === "unauthenticated",
        protectedRoutesBlockUnauthenticated:
            logoutEvidence.loginPageVisible
            && logoutEvidence.dashboardVisible === false,
        loginRouteRemainsPublic: loginDom.loginPageVisible,
        validLoginSucceeds:
            loginEvidence.stateStatus === "authenticated"
            && loginEvidence.accountIdPresent
            && loginEvidence.userAccountIdPresent
            && loginEvidence.accountIdEqualsUserAccountId,
        dashboardAfterLogin: loginEvidence.dashboardVisible,
    };
}

async function runProductInventoryRegressionScenario() {
    return evaluate(cdp, `(async () => {
        ...
        const productRawBefore = localStorage.getItem(productScopedKey);
        const movementRawBefore = localStorage.getItem(stockMovementScopedKey);
        const productsBefore = parseArray(productRawBefore);
        const movementsBefore = parseArray(movementRawBefore);
        const suffix = crypto.randomUUID().slice(0, 8);
        const productId = crypto.randomUUID();
        const salesProductId = crypto.randomUUID();
        const originalName = "V1 REL Product " + suffix;
        const updatedName = "V1 REL Product Updated " + suffix;
        const salesProductName = "V1 REL Sales Product " + suffix;
        const product = {
            id: productId,
            sku: "REL-" + suffix,
            barcode: "RELBC" + suffix,
            name: originalName,
            englishName: "V1 Release Product",
            description: "V1 release regression fixture",
            images: [],
            category: "release",
            brand: "ABONIBAL",
            unit: "pc",
        ...
        const salesProduct = {
            ...product,
            id: salesProductId,
            sku: "REL-SALES-" + suffix,
            barcode: "RELSALESBC" + suffix,
            name: salesProductName,
            englishName: "V1 Release Sales Product",
            quantity: 888,
            createdAt: new Date(),
            updatedAt: new Date()
        };

        const addErrors = productService.add(product);
        const createdProduct = productService.find(productId);
        const salesAddErrors = productService.add(salesProduct);
        const createdSalesProduct = productService.find(salesProductId);
        const updateErrors = productService.update(productId, {
            name: updatedName,
            barcode: "RELBCU" + suffix
        });
        const updatedProduct = productService.find(productId);
        const openingResult = inventoryService.addMovement({
            productId,
            type: "opening_balance",
            quantityDelta: 5,
            reason: "V1-REL-001 opening balance",
            referenceType: "opening_balance",
            metadata: { source: "v1-rel-001" }
        });
        const adjustmentResult = inventoryService.addMovement({
            productId,
            type: "manual_adjustment",
            quantityDelta: -1,
            reason: "V1-REL-001 manual adjustment",
            referenceType: "manual",
            metadata: { source: "v1-rel-001" }
        });
        const quantityAfterAdjustment =
            inventoryService.getCurrentQuantity(productId);
        const voidResult = adjustmentResult.movement
            ? inventoryService.voidMovement(
                adjustmentResult.movement.id,
                "V1-REL-001 void exclusion verification"
            )
            : { success: false, movement: null };
        const quantityAfterVoid = inventoryService.getCurrentQuantity(productId);
        const availabilityFulfilled =
            inventoryService.checkAvailability(productId, 5);
        const availabilityBlocked =
            inventoryService.checkAvailability(productId, 6);
        const salesOpeningResult = inventoryService.addMovement({
            productId: salesProductId,
            type: "opening_balance",
            quantityDelta: 6,
            reason: "V1-REL-001 sales lifecycle opening balance",
            referenceType: "opening_balance",
            metadata: { source: "v1-rel-001-sales-fixture" }
        });
        const productRawAfterInventory =
            localStorage.getItem(productScopedKey);
        const movementRawAfterInventory =
            localStorage.getItem(stockMovementScopedKey);
        const productsAfterInventory = parseArray(productRawAfterInventory);
        const movementsAfterInventory = parseArray(movementRawAfterInventory);
        const storedProduct = productsAfterInventory.find(
            currentProduct => currentProduct?.id === productId
        );
        const storedOpening = movementsAfterInventory.find(
            movement => movement?.id === openingResult.movement?.id
        );
        const storedAdjustment = movementsAfterInventory.find(
            movement => movement?.id === adjustmentResult.movement?.id
        );
        ...
        return {
            ...
            productScopedKeyMatches: productScopedKey === "products:" + accountId,
            stockMovementScopedKeyMatches:
                stockMovementScopedKey === "stockMovements:" + accountId,
            productCountBefore: productsBefore.length,
            stockMovementCountBefore: movementsBefore.length,
            productCreateWorks:
                addErrors.length === 0
                && Boolean(createdProduct)
                && salesAddErrors.length === 0
                && Boolean(createdSalesProduct)
                && createdProduct.accountId === accountId
                && createdProduct.createdBy === userId
                && createdSalesProduct.accountId === accountId,
            productEditWorks:
            ...
            manualOpeningBalanceWorks:
                openingResult.success
                && storedOpening?.type === "opening_balance"
                && storedOpening?.quantityDelta === 5,
            manualAdjustmentWorks:
                adjustmentResult.success
                && storedAdjustment?.type === "manual_adjustment"
                && storedAdjustment?.quantityDelta === -1,
            currentQuantityAfterAdjustment: quantityAfterAdjustment,
            currentQuantityAfterVoid: quantityAfterVoid,
            currentQuantityEqualsLedger: quantityAfterAdjustment === 4,
            voidedMovementsExcluded:
                voidResult.success
                && Boolean(storedAdjustment?.voidedAt)
                && quantityAfterVoid === 5,
            stockAvailabilityGateCorrect:
                availabilityFulfilled.canFulfill === true
                && availabilityFulfilled.availableQuantity === 5,
            stockAvailabilityBlocksOverRequest:
                availabilityBlocked.canFulfill === false
                && availabilityBlocked.status === "insufficient_stock",
            salesLifecycleFixtureReady:
                salesOpeningResult.success
                && inventoryService.getAvailableQuantity(salesProductId) >= 6,
            productQuantityNotUpdatedByInventory:
                storedProduct?.quantity === 777,
            productHashBeforeReleaseCrud: await nullableSha256(productRawBefore),
            productHashAfterInventory:
                await nullableSha256(productRawAfterInventory),
            stockMovementHashBeforeReleaseCrud:
                await nullableSha256(movementRawBefore),
            stockMovementHashAfterInventory:
                await nullableSha256(movementRawAfterInventory),
            productCountAfterInventory: productsAfterInventory.length,
            stockMovementCountAfterInventory: movementsAfterInventory.length,
            inventoryReloadPersistenceWorks:
                Boolean(storedOpening)
                && Boolean(storedAdjustment)
                && Boolean(storedAdjustment?.voidedAt),
        };
    })()`, true);
}

async function exerciseProductSearchFilterScenario(productId, productName) {
    return evaluate(cdp, `(async () => {
        const productId = ${JSON.stringify(productId)};
        const productName = ${JSON.stringify(productName)};
        const searchInput = document.querySelector("#product-search");
        if (!(searchInput instanceof HTMLInputElement)) {
            return { productSearchFilterWorks: false };
        }

        searchInput.value = productName;
        searchInput.dispatchEvent(new Event("input", { bubbles: true }));
        await new Promise(resolve => setTimeout(resolve, 0));
        const matchingRowVisible = Boolean(
            document.querySelector("[data-product-id='" + CSS.escape(productId) + "']")
        );

        searchInput.value = "NO-MATCH-" + productName;
        searchInput.dispatchEvent(new Event("input", { bubbles: true }));
        await new Promise(resolve => setTimeout(resolve, 0));
        const nonMatchingRowHidden = !document.querySelector(
        ...
        return {
            productSearchFilterWorks:
                matchingRowVisible && nonMatchingRowHidden,
            productSearchMatchingRowVisible: matchingRowVisible,
            productSearchNonMatchingRowHidden: nonMatchingRowHidden,
        };
    })()`, true);
}

async function collectInventoryRegressionDomEvidence(productId) {
    return evaluate(cdp, `(() => {
        const productId = ${JSON.stringify(productId)};
        const productRow = document.querySelector(
            "#inventory-products-body [data-product-id='" + CSS.escape(productId) + "']"
        );
        const movementRows = Array.from(
            document.querySelectorAll(
                "#inventory-movement-history-body [data-product-id='" + CSS.escape(productId) + "']"
            )
        );

        return {
            inventoryPageVisible: Boolean(document.querySelector("#inventory-page")),
            currentStockViewRenders:
                Boolean(document.querySelector("#inventory-current-stock-view"))
                && Boolean(productRow),
            movementHistoryRenders:
                Boolean(document.querySelector("#inventory-movement-history-view"))
                && movementRows.length >= 2,
            inventoryCurrentQuantityDom:
                Number(productRow?.getAttribute("data-current-quantity") ?? "NaN"),
            inventoryVoidedMovementVisible:
                movementRows.some(row =>
                    row.getAttribute("data-voided") === "true"
                ),
        };
    })()`, true);
}

async function runProductSafeDeleteScenario(productId) {
    return evaluate(cdp, `(async () => {
        ${browserUtilityFunctions()}
        const { Container } = await import("/src/core/Container.ts");
        const { getAuthStateService } =
        ...
        const productScopedKey = productStorageKeyForAccount(accountId);
        const productId = ${JSON.stringify(productId)};
        const activeCountBefore = productService.getAll().length;
        const errors = productService.safeDelete(productId);
        const activeCountAfter = productService.getAll().length;
        const productRawAfterDelete = localStorage.getItem(productScopedKey);
        const productsAfterDelete = parseArray(productRawAfterDelete);
        const deletedProduct = productsAfterDelete.find(
            product => product?.id === productId
        );

        return {
            productSafeDeleteWorks:
                errors.length === 0
                && Boolean(deletedProduct?.isDeleted)
                && Boolean(deletedProduct?.deletedAt)
                && deletedProduct?.accountId === accountId,
            productActiveCountBeforeDelete: activeCountBefore,
            productActiveCountAfterDelete: activeCountAfter,
            productReloadPersistenceWorks:
                Boolean(deletedProduct?.isDeleted)
                && activeCountAfter === activeCountBefore - 1,
            productHashAfterSafeDelete:
                await nullableSha256(productRawAfterDelete),
            productCountAfterSafeDelete: productsAfterDelete.length,
            softDeletedProductQuantityPreserved:
                deletedProduct?.quantity === 777,
        };
    })()`, true);
}

async function collectProductAfterDeleteDomEvidence(productId) {
    return evaluate(cdp, `(() => {
        const productId = ${JSON.stringify(productId)};
        return {
            softDeletedProductHiddenFromUi: !document.querySelector(
                "[data-product-id='" + CSS.escape(productId) + "']"
            ),
        };
    })()`, true);
...
async function collectBaselineSnapshot() {
    return evaluate(cdp, `(async () => {
        ...
        const invoiceReturnRaw = localStorage.getItem(invoiceReturnScopedKey);
        ...
        const invoiceReturns = parseArray(invoiceReturnRaw);
        ...
        return {
            ...
            invoiceReturnRawHash: await nullableSha256(invoiceReturnRaw),
            ...
            invoiceReturnCount: invoiceReturns.length,
...
async function collectLoginDomEvidence() {
    return evaluate(cdp, `(() => ({
        url: location.href,
        title: document.title,
        loginPageVisible: Boolean(document.querySelector("#login-page")),
        loginFormVisible: Boolean(document.querySelector("#login-form")),
        logoutButtonVisible: Boolean(document.querySelector("#logout-button")),
        authStatus:
            document.querySelector("#login-page")?.getAttribute("data-auth-status")
            ?? "",
    }))()`);
}

async function collectDashboardDomEvidence() {
    return evaluate(cdp, `(() => {
        ...
        return {
            ...
            mainNavigationVisible:
                ...
                && document.querySelectorAll(".menu-item").length >= 5,
            navigationItems:
...
async function collectDomEvidence(extra = {}) {
    return evaluate(cdp, `(() => {
        ...
        const returnInputs = Array.from(
            document.querySelectorAll(".invoice-return-quantity-input")
        );
        const returnAudits = Array.from(
        ...
        return {
            ...
            returnQuantityInputCount: returnInputs.length,
            returnAuditText: returnAudits
                .map(element => element.textContent?.trim() ?? "")
                .join(" "),
            draftReturnActionExists:
                actionButtons.some(button =>
                    button.getAttribute("data-action") === "create-invoice-return"
                    && Array.from(document.querySelectorAll("[data-invoice-status='draft']"))
                        .some(row => row.getAttribute("data-invoice-id")
                            === button.getAttribute("data-invoice-id"))
                ),
            cancelledReturnActionExists:
                actionButtons.some(button =>
                    button.getAttribute("data-action") === "create-invoice-return"
                    && Array.from(document.querySelectorAll("[data-invoice-status='cancelled']"))
                        .some(row => row.getAttribute("data-invoice-id")
                            === button.getAttribute("data-invoice-id"))
                ),
...
async function runLifecycleScenario() {
    return evaluate(cdp, `(async () => {
        ...
        const beforeInvoiceReturnRaw =
            localStorage.getItem(invoiceReturnScopedKey);
        ...
        const invoiceReturnsBefore = invoiceReturnService.getAll();
        ...
        const invoiceReturnCountBefore = invoiceReturnsBefore.length;
        ...
        const createResult = invoiceService.createDraft({
            customerSnapshot: { displayName: "V1 REL 001 Customer" },
            lines: [lineInput(1)],
            notes: "V1-REL-001 lifecycle draft",
        });
        ...
        const updateResult = invoiceService.updateDraft(draft.id, {
            lines: [lineInput(2)],
            notes: "V1-REL-001 lifecycle draft updated",
        });
        ...
        const failedDraftResult = invoiceService.createDraft({
            customerSnapshot: { displayName: "V1 REL 001 Insufficient Stock" },
            lines: [lineInput(availabilityBeforeIssue + 100)],
            notes: "V1-REL-001 insufficient quantity fixture",
        });
        ...
        const draftCancelResult = invoiceService.markCancelled(
            failedDraftResult.invoice.id,
            "V1-REL-001 draft cancellation should remain blocked"
        );
        ...
        const cancelResult = invoiceService.markCancelled(
            issuedInvoice.id,
            "V1-REL-001 lifecycle cancellation"
        );
        ...
        const duplicateCancelResult = invoiceService.markCancelled(
            cancelledInvoice.id,
            "V1-REL-001 duplicate cancellation should remain blocked"
        );
        ...
        const returnDraftResult = invoiceService.createDraft({
            customerSnapshot: { displayName: "V1 SALES 014 Return Customer" },
            lines: [lineInput(2)],
            notes: "V1-REL-001 return UI invoice fixture",
        });

        if (!returnDraftResult.success || !returnDraftResult.invoice) {
            throw new Error("Return invoice draft create failed: " + returnDraftResult.errors.join("; "));
        }

        const returnIssueMovementCountBefore = inventoryService.getAll().length;
        const returnIssueResult = invoiceService.markIssued(
            returnDraftResult.invoice.id
        );

        if (!returnIssueResult.success || !returnIssueResult.invoice) {
            throw new Error("Return invoice issue failed: " + returnIssueResult.errors.join("; "));
        }

        const returnInvoice = returnIssueResult.invoice;
        const returnInvoiceLine = returnInvoice.lines[0];
        const movementsAfterReturnIssue = inventoryService.getAll();
        const returnSaleDeduction = movementsAfterReturnIssue.find(movement =>
            movement.id === returnInvoiceLine.stockMovementId
        );
        const availabilityBeforeReturn =
            inventoryService.getAvailableQuantity(targetProduct.id);
        const returnInvoiceIssued =
            returnInvoice.status === "issued"
            && returnIssueResult.success === true;
        const returnInvoiceLineStockMovementIdSet =
            typeof returnInvoiceLine.stockMovementId === "string"
            && returnInvoiceLine.stockMovementId.trim().length > 0;
        const returnSaleDeductionCreated =
            movementsAfterReturnIssue.length === returnIssueMovementCountBefore + 1
            && returnSaleDeduction?.type === "sale_deduction";

        const finalInvoices = invoiceService.getAll();
        const finalInvoiceReturns = invoiceReturnService.getAll();
        const finalMovements = inventoryService.getAll();
        const finalInvoiceRaw = localStorage.getItem(invoiceScopedKey);
        const finalInvoiceReturnRaw =
            localStorage.getItem(invoiceReturnScopedKey);
        ...
        const stockMovementsOnlyExpectedChanges =
            finalMovements.length === movementCountBefore + 3
            && finalSaleDeductionCount === saleDeductionCountBefore + 2
            && finalSaleReturnCount === saleReturnCountBefore + 1;
        const invoicesOnlyExpectedLifecycleWrites =
            finalInvoices.length === invoiceCountBefore + 3;
        const invoiceReturnsUnchangedBeforeReturnUi =
            finalInvoiceReturns.length === invoiceReturnCountBefore;
        ...
        return {
            ...
            returnInvoiceIdSanitized: await sanitizeValue(returnInvoice.id),
            returnInvoiceLineIdSanitized:
                await sanitizeValue(returnInvoiceLine.id),
            saleDeductionMovementIdSanitized:
                await sanitizeValue(saleDeduction?.id),
            saleReturnMovementIdSanitized:
                await sanitizeValue(saleReturn?.id),
            returnSaleDeductionMovementIdSanitized:
                await sanitizeValue(returnSaleDeduction?.id),
            ...
            _rawReturnInvoiceId: returnInvoice.id,
            _rawReturnInvoiceLineId: returnInvoiceLine.id,
            ...
            _rawReturnSaleDeductionMovementId: returnSaleDeduction?.id ?? "",
            invoiceCountBefore,
            invoiceCountAfter: finalInvoices.length,
            invoiceReturnCountBefore,
            invoiceReturnCountAfterBeforeUi: finalInvoiceReturns.length,
            movementCountBefore,
            ...
            invoiceReturnRawHashBefore:
                await nullableSha256(beforeInvoiceReturnRaw),
            invoiceReturnRawHashAfterBeforeUi:
                await nullableSha256(finalInvoiceReturnRaw),
            ...
            duplicateCancellationLeavesCancelled,
            returnInvoiceIssued,
            returnInvoiceLineStockMovementIdSet,
            returnSaleDeductionCreated,
            returnSaleDeductionProductMatches:
                returnSaleDeduction?.productId === targetProduct.id,
            returnSaleDeductionNegative:
                typeof returnSaleDeduction?.quantityDelta === "number"
                && returnSaleDeduction.quantityDelta < 0,
            returnSaleDeductionReferencesInvoice:
                returnSaleDeduction?.referenceId === returnInvoice.id,
            stockMovementsOnlyExpectedChanges,
            invoicesOnlyExpectedLifecycleWrites,
            invoiceReturnsUnchangedBeforeReturnUi,
            softDeletedProductsNotSelectable,
        };
    })()`, true);
}
...
async function exerciseReturnUiScenario(lifecycle) {
    return evaluate(cdp, `(async () => {
        ${browserUtilityFunctions()}
        const { Container } = await import("/src/core/Container.ts");
        const { getAuthStateService } =
        ...
        const state = await getAuthStateService().initialize();
        if (state.status !== "authenticated") {
            throw new Error("Authenticated session is required for return UI verification.");
        }
        ...
        const invoiceReturnService = Container.get("invoiceReturnService");
        const inventoryService = Container.get("inventoryService");
        const returnInvoiceId = ${JSON.stringify(lifecycle._rawReturnInvoiceId)};
        const returnInvoiceLineId =
            ${JSON.stringify(lifecycle._rawReturnInvoiceLineId)};
        const returnSaleDeductionMovementId =
            ${JSON.stringify(lifecycle._rawReturnSaleDeductionMovementId)};
        const productId = ${JSON.stringify(lifecycle._rawProductId)};
        const invoiceScopedKey = invoiceStorageKeyForAccount(accountId);
        const invoiceReturnScopedKey =
            invoiceReturnStorageKeyForAccount(accountId);
        ...
        const beforeProductRaw = localStorage.getItem(productScopedKey);
        const beforeLegacyProductRaw = localStorage.getItem("products");
        const beforeReturns = parseArray(beforeInvoiceReturnRaw);
        const beforeMovements = parseArray(beforeMovementRaw);
        const beforeReturnIds = new Set(beforeReturns.map(item => item.id));
        const beforeMovementIds = new Set(beforeMovements.map(item => item.id));
        const returnInvoice = invoiceService.getById(returnInvoiceId);
        const returnLine = returnInvoice?.lines?.find(line =>
            line.id === returnInvoiceLineId
        );
        ...
        const invalidAttempt = submitReturnViaUi(returnInvoiceLineId, 0);
        const afterInvalid = collectStorage({
            invoiceScopedKey,
            invoiceReturnScopedKey,
            stockMovementScopedKey,
            productScopedKey,
        });
        const overReturnAttempt = submitReturnViaUi(
            returnInvoiceLineId,
            remainingBefore + 1
        );
        const afterOverReturn = collectStorage({
            invoiceScopedKey,
            invoiceReturnScopedKey,
            stockMovementScopedKey,
            productScopedKey,
        });
        const validReturnQuantity = Math.min(1, remainingBefore);
        const validAttempt = submitReturnViaUi(
            returnInvoiceLineId,
            validReturnQuantity
        );
        const afterValid = collectStorage({
            invoiceScopedKey,
            invoiceReturnScopedKey,
            stockMovementScopedKey,
            productScopedKey,
        });
        const createdReturn = afterValid.returns.find(invoiceReturn =>
            !beforeReturnIds.has(invoiceReturn.id)
        );
        const createdReturnLine = createdReturn?.lines?.find(line =>
            line.invoiceLineId === returnInvoiceLineId
        );
        const createdSaleReturn = afterValid.movements.find(movement =>
            !beforeMovementIds.has(movement.id)
            && movement.id === createdReturnLine?.returnStockMovementId
        );
        const remainingAfterValid =
            invoiceReturnService.getRemainingReturnableQuantity(
                returnInvoiceId,
                returnInvoiceLineId
            );
        const duplicateExcessiveAttempt = submitReturnViaUi(
            returnInvoiceLineId,
            remainingAfterValid + 1
        );
        const afterDuplicate = collectStorage({
            invoiceScopedKey,
            invoiceReturnScopedKey,
            stockMovementScopedKey,
            productScopedKey,
        });
        const availabilityAfterReturn =
            inventoryService.getAvailableQuantity(productId);
        ...
        return {
            returnUiRendersForIssuedInvoice: uiBefore.returnButtonExists,
            returnUiDoesNotRenderForDraftInvoice:
                uiBefore.draftReturnActionExists === false,
            returnUiDoesNotRenderForCancelledInvoice:
                uiBefore.cancelledReturnActionExists === false,
            returnableInvoiceLinesRender:
                uiBefore.returnRowExists && Boolean(returnLine),
            remainingReturnableDisplayed:
                uiBefore.remainingText.includes(String(remainingBefore)),
            invalidReturnQuantityRejected:
                invalidAttempt.message.includes("Return quantity")
                && afterInvalid.returnRaw === beforeInvoiceReturnRaw
                && afterInvalid.movementRaw === beforeMovementRaw,
            overReturnRejected:
                overReturnAttempt.message.includes("exceeds remaining")
                && afterOverReturn.returnRaw === beforeInvoiceReturnRaw
                && afterOverReturn.movementRaw === beforeMovementRaw,
            duplicateExcessiveReturnRejected:
                duplicateExcessiveAttempt.message.includes("exceeds remaining")
                && afterDuplicate.returns.length === afterValid.returns.length
                && afterDuplicate.movements.length === afterValid.movements.length,
            validPartialReturnCreatesRecord: Boolean(createdReturn),
            validPartialReturnExecutes:
                createdReturn?.status === "executed",
            returnSaleReturnMovementCreated:
                createdSaleReturn?.type === "sale_return",
            returnSaleReturnReferenceTypeInvoiceReturn:
            ...
            returnSaleReturnReferencesOriginal:
                createdSaleReturn?.metadata?.originalSaleDeductionMovementId
                    === returnSaleDeductionMovementId
                || createdSaleReturn?.metadata?.originalStockMovementId
                    === returnSaleDeductionMovementId
                || createdSaleReturn?.metadata?.reversesMovementId
                    === returnSaleDeductionMovementId,
            availabilityBeforeReturn,
            availabilityAfterReturn,
            returnQuantity: createdReturnLine?.returnQuantity ?? 0,
            availableStockIncreasesByReturnedQuantity:
                availabilityAfterReturn === availabilityBeforeReturn
                    + (createdReturnLine?.returnQuantity ?? 0),
            remainingReturnableBeforeReturn: remainingBefore,
            remainingReturnableAfterReturn: remainingAfterValid,
            remainingReturnableDecreasesCorrectly:
                remainingAfterValid === remainingBefore
                    - (createdReturnLine?.returnQuantity ?? 0),
            previousReturnsDisplayAfterExecution:
                uiAfter.auditText.includes(createdReturn?.returnNumber ?? "")
                && uiAfter.auditText.includes(
                    createdReturnLine?.returnStockMovementId ?? ""
                ),
            returnedInvoiceIdSanitized: await sanitizeValue(returnInvoiceId),
            returnInvoiceIdSanitized: await sanitizeValue(returnInvoiceId),
            createdInvoiceReturnIdSanitized:
                await sanitizeValue(createdReturn?.id),
            createdReturnSaleReturnMovementIdSanitized:
                await sanitizeValue(createdSaleReturn?.id),
            _rawCreatedInvoiceReturnId: createdReturn?.id ?? "",
            _rawCreatedReturnSaleReturnMovementId:
                createdSaleReturn?.id ?? "",
            invoiceReturnCountAfterReturn: afterValid.returns.length,
            invoiceReturnCountAfterDuplicate: afterDuplicate.returns.length,
            movementCountAfterReturn: afterValid.movements.length,
            movementCountAfterDuplicateReturn: afterDuplicate.movements.length,
            invoiceHashAfterReturn: await nullableSha256(afterValid.invoiceRaw),
            invoiceReturnHashAfterReturn:
                await nullableSha256(afterValid.returnRaw),
            movementHashAfterReturn:
                await nullableSha256(afterValid.movementRaw),
            productHashAfterReturn: await nullableSha256(afterValid.productRaw),
            legacyProductHashAfterReturn:
                await nullableSha256(afterValid.legacyProductRaw),
            invoiceHashUnchangedByReturn:
                await nullableSha256(beforeInvoiceRaw)
                    === await nullableSha256(afterValid.invoiceRaw),
            productHashUnchangedByReturn:
                await nullableSha256(beforeProductRaw)
                    === await nullableSha256(afterValid.productRaw),
            legacyProductHashUnchangedByReturn:
                await nullableSha256(beforeLegacyProductRaw)
                    === await nullableSha256(afterValid.legacyProductRaw),
            returnAuditVisible:
                uiAfter.auditText.includes(createdReturn?.returnNumber ?? "")
                && uiAfter.auditText.includes(
        ...
        function collectReturnDomEvidence(invoiceId, invoiceLineId) {
            const returnRows = Array.from(
                document.querySelectorAll(".invoice-return-line-row")
            );
            ...
            const matchingRow = returnRows.find(row =>
                row.getAttribute("data-invoice-id") === invoiceId
                && row.getAttribute("data-invoice-line-id") === invoiceLineId
            );
            const matchingButton = returnButtons.find(button =>
                button.getAttribute("data-invoice-id") === invoiceId
                && button.getAttribute("data-invoice-line-id") === invoiceLineId
            );
            const matchingInput = returnInputs.find(input =>
                input.getAttribute("data-invoice-id") === invoiceId
                && input.getAttribute("data-invoice-line-id") === invoiceLineId
            );
            const statusesByInvoiceId = new Map(
                Array.from(document.querySelectorAll(".invoice-draft-row"))
                    .map(row => [
                        row.getAttribute("data-invoice-id"),
                        row.getAttribute("data-invoice-status"),
                    ])
            );

            return {
                returnRowExists: Boolean(matchingRow),
                returnButtonExists: Boolean(matchingButton),
                returnInputExists: Boolean(matchingInput),
                remainingText:
                    matchingRow?.querySelector(".invoice-return-remaining")
                        ?.textContent?.trim() ?? "",
                auditText: returnAudits
                    .map(element => element.textContent?.trim() ?? "")
                    .join(" "),
                draftReturnActionExists: returnButtons.some(button =>
                    statusesByInvoiceId.get(
                        button.getAttribute("data-invoice-id")
                    ) === "draft"
                ),
                cancelledReturnActionExists: returnButtons.some(button =>
                    statusesByInvoiceId.get(
                        button.getAttribute("data-invoice-id")
                    ) === "cancelled"
                ),
...
async function collectReturnReloadEvidence(lifecycle) {
    return evaluate(cdp, `(async () => {
        const { Container } = await import("/src/core/Container.ts");
        const invoiceReturnService = Container.get("invoiceReturnService");
        const inventoryService = Container.get("inventoryService");
        const productId = ${JSON.stringify(lifecycle._rawProductId)};
        const invoiceReturnId =
            ${JSON.stringify(lifecycle._rawCreatedInvoiceReturnId)};
        const saleReturnMovementId =
            ${JSON.stringify(lifecycle._rawCreatedReturnSaleReturnMovementId)};
        const invoiceReturn = invoiceReturnService.getById(invoiceReturnId);
        const movements = inventoryService.getAll();
        const saleReturn = movements.find(movement =>
            movement.id === saleReturnMovementId
        );
        const auditText = Array.from(
            document.querySelectorAll(".invoice-return-audit")
        ).map(element => element.textContent?.trim() ?? "").join(" ");

        return {
            reloadPreservesInvoiceReturnRecord:
                invoiceReturn?.status === "executed",
            reloadPreservesReturnSaleReturn:
                saleReturn?.type === "sale_return",
            reloadPreservesReturnAudit:
                auditText.includes(invoiceReturn?.returnNumber ?? "")
                && auditText.includes(saleReturnMovementId),
            availabilityAfterReturnReload:
                inventoryService.getAvailableQuantity(productId),
        };
    })()`, true);
    ...
    return evaluate(cdp, `(async () => {
        ...
        const draftResult = draftInvoice
            ? invoiceService.markCancelled(
                draftInvoice.id,
                "V1-REL-001 draft cancellation should fail"
            )
        ...
        const missingResult = invoiceService.markCancelled(
            ...
            "V1-REL-001 missing invoice should fail"
        );
...
async function exerciseDuplicateCancellationCase() {
    return evaluate(cdp, `(async () => {
        ...
        const duplicateResult = invoiceService.markCancelled(
            cancelledInvoice.id,
            "V1-REL-001 duplicate cancellation should fail"
        );
        const movementCountAfter = inventoryService.getAll().length;
...
function buildSnapshotSummary(before, after) {
    return {
        ...
        invoiceReturnsScopedKey: before.invoiceReturnScopedKeySanitized,
        ...
        invoiceReturnCountBefore: before.invoiceReturnCount,
        invoiceReturnCountAfter: after.invoiceReturnCount,
        stockMovementCountBefore: before.stockMovementCount,
        stockMovementCountAfter: after.stockMovementCount,
        invoiceReturnHashBefore: before.invoiceReturnRawHash,
        invoiceReturnHashAfter: after.invoiceReturnRawHash,
...
function buildLifecycleSummary(
    ...
    return {
        ...
        dashboardNavigationResult: {
            dashboardVisible: dashboardDom.dashboardVisible,
            mainNavigationVisible: dashboardDom.mainNavigationVisible,
            navigationItems: dashboardDom.navigationItems,
        },
        productRegressionResult: {
            productCreateWorks: productInventory.productCreateWorks,
            productEditWorks: productInventory.productEditWorks,
            productSearchFilterWorks:
                productInventory.productSearchFilterWorks,
            productSafeDeleteWorks: productInventory.productSafeDeleteWorks,
            softDeletedProductHiddenFromUi:
                productInventory.softDeletedProductHiddenFromUi,
            productReloadPersistenceWorks:
                productInventory.productReloadPersistenceWorks,
            productQuantityPreserved:
                productInventory.softDeletedProductQuantityPreserved,
            productCountBeforeReleaseCrud:
                productInventory.productCountBefore,
            productCountAfterSafeDelete:
                productInventory.productCountAfterSafeDelete,
        },
        inventoryRegressionResult: {
            inventoryPageVisible: productInventory.inventoryPageVisible,
            manualOpeningBalanceWorks:
                productInventory.manualOpeningBalanceWorks,
            manualAdjustmentWorks: productInventory.manualAdjustmentWorks,
            currentStockViewRenders:
                productInventory.currentStockViewRenders,
            movementHistoryRenders: productInventory.movementHistoryRenders,
            currentQuantityAfterAdjustment:
                productInventory.currentQuantityAfterAdjustment,
            currentQuantityAfterVoid:
                productInventory.currentQuantityAfterVoid,
            currentQuantityEqualsLedger:
                productInventory.currentQuantityEqualsLedger,
            voidedMovementsExcluded:
                productInventory.voidedMovementsExcluded,
        ...
        returnInvoiceId: lifecycle.returnInvoiceIdSanitized,
        returnedInvoiceId: lifecycle.returnedInvoiceIdSanitized,
        saleDeductionMovementId:
            lifecycle.saleDeductionMovementIdSanitized,
        saleReturnCancellationMovementId:
            lifecycle.saleReturnMovementIdSanitized,
        saleReturnReturnMovementId:
            lifecycle.createdReturnSaleReturnMovementIdSanitized,
        ...
        duplicateReturnResult: {
            blocked: lifecycle.duplicateExcessiveReturnRejected,
            returnCountStable:
                lifecycle.invoiceReturnCountAfterDuplicate
                === lifecycle.invoiceReturnCountAfterReturn,
            movementCountStable:
        ...
        overReturnRejectionResult: lifecycle.overReturnRejected,
        ...
        availabilityAfterReturn: lifecycle.availabilityAfterReturn,
        availabilityAfterReload: lifecycle.availabilityAfterReturnReload,
        invoiceCountBefore: lifecycle.invoiceCountBefore,
        invoiceCountAfter: lifecycle.invoiceCountAfter,
        invoiceReturnCountBefore: lifecycle.invoiceReturnCountBefore,
        invoiceReturnCountAfter: lifecycle.invoiceReturnCountAfterReturn,
        movementCountBefore: lifecycle.movementCountBefore,
        movementCountAfter: lifecycle.movementCountAfterReturn,
        ...
        returnUiResult: {
            rendersForIssuedInvoice: lifecycle.returnUiRendersForIssuedInvoice,
            hiddenForDraftInvoice: lifecycle.returnUiDoesNotRenderForDraftInvoice,
            hiddenForCancelledInvoice:
                lifecycle.returnUiDoesNotRenderForCancelledInvoice,
            returnableLinesRender: lifecycle.returnableInvoiceLinesRender,
            remainingDisplayed: lifecycle.remainingReturnableDisplayed,
        },
        returnExecutionResult: {
            invalidRejected: lifecycle.invalidReturnQuantityRejected,
            overReturnRejected: lifecycle.overReturnRejected,
            duplicateExcessiveRejected:
                lifecycle.duplicateExcessiveReturnRejected,
            recordCreated: lifecycle.validPartialReturnCreatesRecord,
            executed: lifecycle.validPartialReturnExecutes,
            saleReturnCreated: lifecycle.returnSaleReturnMovementCreated,
            stockMovementIdSet: lifecycle.returnStockMovementIdSet,
            stockMovementIdReferencesMovement:
                lifecycle.returnStockMovementIdReferencesMovement,
            remainingReturnableBefore:
                lifecycle.remainingReturnableBeforeReturn,
            remainingReturnableAfter:
                lifecycle.remainingReturnableAfterReturn,
            remainingDecreasesCorrectly:
                lifecycle.remainingReturnableDecreasesCorrectly,
        },
        auditViewResult: {
        ...
        reloadPersistenceResult: {
            ...
            invoiceReturn: lifecycle.reloadPreservesInvoiceReturnRecord,
            returnSaleReturn: lifecycle.reloadPreservesReturnSaleReturn,
            returnAudit: lifecycle.reloadPreservesReturnAudit,
...
function sanitizeLifecycleStorageSnapshot(before, lifecycle, productInventory) {
    return {
        ...
        productCountAfter: productInventory.productCountAfterSafeDelete,
        stockMovementCountBefore: before.stockMovementCount,
        stockMovementCountAfter: lifecycle.movementCountAfterReturn,
        productScopedHashBefore: before.productScopedHash,
        productScopedHashAfter: lifecycle.productHashAfterReturn,
        legacyProductHashBefore: lifecycle.legacyProductHashBefore,
        legacyProductHashAfter: lifecycle.legacyProductHashAfterReturn,
        invoiceRawHashBefore: lifecycle.invoiceRawHashBefore,
        invoiceRawHashAfter: lifecycle.invoiceHashAfterReturn,
        invoiceReturnRawHashBefore: lifecycle.invoiceReturnRawHashBefore,
        invoiceReturnRawHashAfter: lifecycle.invoiceReturnHashAfterReturn,
        stockMovementRawHashBefore: lifecycle.stockMovementRawHashBefore,
        stockMovementRawHashAfter: lifecycle.movementHashAfterReturn,
        duplicateIssueResult:
            lifecycle.duplicateIssueCreatesNoDuplicateSaleDeduction,
        duplicateCancellationResult:
            lifecycle.duplicateCancellationCreatesNoDuplicateSaleReturn,
        duplicateReturnResult: lifecycle.duplicateExcessiveReturnRejected,
        overReturnRejectionResult: lifecycle.overReturnRejected,
        availabilityBeforeIssue: lifecycle.availabilityBeforeIssue,
        availabilityAfterIssue: lifecycle.availabilityAfterIssue,
        availabilityAfterCancellation: lifecycle.availabilityAfterCancellation,
...
function writeFullV1ReleaseCandidateSummary(summary) {
    writeFileSync(
        resolve(outputDir, "full-v1-release-candidate-summary.json"),
        JSON.stringify(summary, null, 2)
    );
}
...
async function captureVisualSnapshots() {
    ...
    await navigateRoute(cdp, "login");
    await waitForSelector(cdp, "#login-page", 30000);
    screenshots.push(await captureNamedScreenshot("login-page.png"));

    await navigateRoute(cdp, "dashboard");
    await waitForSelector(cdp, "h1", 30000);
    screenshots.push(await captureNamedScreenshot("dashboard.png"));

    await navigateRoute(cdp, "products");
    await waitForSelector(cdp, ".products-page", 30000);
    screenshots.push(await captureNamedScreenshot("products-page.png"));

    await navigateRoute(cdp, "inventory");
    await waitForSelector(cdp, "#inventory-page", 30000);
    screenshots.push(await captureNamedScreenshot("inventory-page.png"));

    await navigateRoute(cdp, "invoices");
    await waitForSelector(cdp, "#invoice-draft-page", 30000);
    screenshots.push(await captureNamedScreenshot("invoice-draft-page.png"));
    screenshots.push(await captureNamedScreenshot("issued-invoice-audit.png"));
    screenshots.push(await captureNamedScreenshot("cancelled-invoice-audit.png"));
    screenshots.push(await captureNamedScreenshot("invoice-return-ui.png"));

    return screenshots;
}

async function captureNamedScreenshot(fileName) {
    const screenshotsDir = resolve(outputDir, "screenshots");
    ...
    return `outputs/V1-REL-001/screenshots/${fileName}`;
}
...
async function ensureAuthenticatedSession(connection) {
    const state = await evaluate(connection, `(async () => {
        const { getAuthStateService } =
            await import("/src/modules/auth/AuthRuntime.ts");
        const authStateService = getAuthStateService();
        const state = await authStateService.initialize();
        return state.status;
    })()`, true);
    ...
    assert(authTestEmail, "ABONIBAL_AUTH_TEST_EMAIL is required for release authentication.");
    assert(authTestPassword, "ABONIBAL_AUTH_TEST_PASSWORD is required for release authentication.");

    await evaluate(connection, `(async () => {
        const { getAuthStateService } =
            await import("/src/modules/auth/AuthRuntime.ts");
        await getAuthStateService().signIn({
            email: ${JSON.stringify(authTestEmail)},
            password: ${JSON.stringify(authTestPassword)}
        });
        return true;
    ...
    await waitForAuthenticated(connection);
}
